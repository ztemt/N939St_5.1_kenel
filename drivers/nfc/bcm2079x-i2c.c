...
#include <linux/nfc/bcm2079x-i2c.h>
...
static void set_client_addr(struct bcm2079x_dev *bcm2079x_dev, int addr)
{
	...
	printk("[bcm2079x]Set client device changed to (0x%04X) flag = %04x \r\n",
...
static void change_client_addr(struct bcm2079x_dev *bcm2079x_dev, int addr)
{
	...
	printk("[bcm2079x]Change client device from (0x%04X) flag = "\
		 "%04x, addr_data[%d] = %02x \\n",
	...
	printk("[bcm2079x]Change client device changed to (0x%04X) flag = %04x, ret = %d\n",
...
static unsigned int bcm2079x_dev_poll(struct file *filp, poll_table *wait)
{
	...
	{
		spin_unlock_irqrestore(&bcm2079x_dev->irq_enabled_lock, flags);
		//printk("[bcm2079x]: bcm2079x_dev_poll, irq count %d, irq_gpio %d\n", bcm2079x_dev->count_irq,  bcm2079x_dev->irq_gpio  );
	...
	}else
	{
		...
		spin_unlock_irqrestore(&bcm2079x_dev->irq_enabled_lock, flags);
		//printk("[bcm2079x]WK_NFC: poll there is data to read!!! no wait any more.\n");
				  ...
				  size_t count, loff_t *offset)
{
	...
	**/
	...
	if (total > count || copy_to_user(buf, tmp, total)) {
		printk("[bcm2079x]failed to copy to user space, total = %d\n", total);
		total = -EFAULT;
				   ...
				   size_t count, loff_t *offset)
{
	...
	if (count > MAX_BUFFER_SIZE) {
		printk("[bcm2079x]out of memory\n");
		return -ENOMEM;
	}

	if (copy_from_user(tmp, buf, count)) {
		printk("[bcm2079x]failed to copy from user space\n");
		return -EFAULT;
	}
...
#endif
		{
			printk("[bcm2079x]failed to write %d\n", ret);
			ret = -EIO;
			bcm2079x_dev->error_write++;
		}
	}
...
static int bcm2079x_dev_open(struct inode *inode, struct file *filp)
{
	...
	printk("[bcm2079x]device node major=%d, minor=%d\n", imajor(inode), iminor(inode));
	return ret;
}
					 ...
					 unsigned int cmd, unsigned long arg)
{
	...
	case BCMNFC_CHANGE_ADDR:
		printk("[bcm2079x]%s, BCMNFC_CHANGE_ADDR (%x, %lx):\n", __func__, cmd,
			 arg);
		change_client_addr(bcm2079x_dev, arg);
		break;
	case BCMNFC_POWER_CTL:
		printk("[bcm2079x]%s, BCMNFC_POWER_CTL (%x, %lx):\n", __func__, cmd,
			 arg);
        if (arg != 1)
            set_client_addr(bcm2079x_dev, bcm2079x_dev->original_address);
	...
	case BCMNFC_WAKE_CTL:
		printk("[bcm2079x]%s, BCMNFC_WAKE_CTL (%x, %lx):\n", __func__, cmd,
			 arg);
#ifdef USE_WAKE_LOCK
		if(arg != 0)
		{
		...
		break;
	default:
		printk("[bcm2079x]%s, unknown cmd (%x, %lx)\n", __func__, cmd, arg);
		return 0;
	}
...
static long bcm2079x_dev_compat_ioctl(struct file *filp,
                                         unsigned int cmd, unsigned long arg)
{
        struct bcm2079x_dev *bcm2079x_dev = filp->private_data;
...
#define PINCTRL_STATE_ACTIVE	"nfc_active"
#define PINCTRL_STATE_SUSPEND	"nfc_suspend"
static int nfc_pinctrl_init(struct i2c_client *client)
{
	int retval;
	...
	pinctrl_state_active = pinctrl_lookup_state(
			ts_pinctrl, PINCTRL_STATE_ACTIVE);
	if (IS_ERR_OR_NULL(pinctrl_state_active)) {
		retval = PTR_ERR(pinctrl_state_active);
		printk("Can't lookup %s pinstate %d\n",
			PINCTRL_STATE_ACTIVE, retval);
		goto err_pinctrl_lookup;
	}

	pinctrl_state_suspend = pinctrl_lookup_state(
			ts_pinctrl, PINCTRL_STATE_SUSPEND);
	...
	retval = pinctrl_select_state(ts_pinctrl, pinctrl_state_active);
	if (retval < 0)
		printk(" [bcm2079x] Failed to select %p %p %p\n",
				ts_pinctrl, pinctrl_state_active,pinctrl_state_suspend);
			   ...
			   const struct i2c_device_id *id)
{
	...
	printk("[bcm2079x]%s, probing bcm2079x driver entry successfully \r\n",__func__);
       nfc_pinctrl_init(client);
	if (client->dev.of_node) {
		...
		if (!platform_data) {
			...
			printk("[bcm2079x]nfc-nci probe: Failed to allocate memory \r\n");
			return -ENOMEM;
		}
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		printk("[bcm2079x] need I2C_FUNC_I2C \r\n");
		return -ENODEV;
	}
    ...
    if (!strcmp(platform_data->clk_src, "BBCLK2")) {
		...
		if (nfc_clk == NULL)
			printk("[bcm2079x]failed to clk_get ref_clk \r\n");
	}
	else
	{
		nfc_clk = NULL;
...
/* Liuyk modified on 2014.07.02 begin */
	...
	if (r)
		dev_err(&client->dev,"[bcm2079x]failed to clk_prepare_enable %d \r\n",r);
#endif
/* Liuyk modified on 2014.07.02 end */
  
	bcm2079x_dev = kzalloc(sizeof(*bcm2079x_dev), GFP_KERNEL);
	if (bcm2079x_dev == NULL) {
		//dev_err(&client->dev, "[bcm2079x]failed to allocate memory for module data \r\n");
		printk("[bcm2079x]failed to allocate memory for module data \r\n");
		ret = -ENOMEM;
		goto err_exit;
	}
	...
	printk("[bcm2079x]%s, probing bcm2079x driver exited successfully\n", __func__);
