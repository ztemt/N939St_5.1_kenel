...
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	call_atten_cb(cd, CY_ATTEN_SUSPEND, 0);          //zhongzhu add  to clean the old point  remained
...
exit:
	...
	call_atten_cb(cd, CY_ATTEN_RESUME, 0);           //zhongzhu add  to clean the old point remained
