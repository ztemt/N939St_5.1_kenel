		...
		int fw_size)
{
	...
	if (rc < 0) {
		dev_err(dev, "%s: Firmware update failed with error code %d\n",
			__func__, rc);
	} else{ //else if (ld->loader_pdata && (ld->loader_pdata->flags & CY_LOADER_FLAG_CALIBRATE_AFTER_FW_UPGRADE)) {
 ...
 */
   ...
   //cyttsp5_fw_pid00.bin.ihex 	     liangchuang           05 project
   //cyttsp5_fw_pid01.bin.ihex		yeji 05 project
   //cyttsp5_fw_pid02.bin.ihex 	     yeji            04 project
...
#define FILENAME_LEN_MAX 64
	...
	if (panel_id == PANEL_ID_NOT_ENABLED){		
		...
		#ifdef CONFIG_TOUCHSCREEN_CYPRESS_05_PROJECT
		panel_id = 0x01;
		printk("xxxxxxxxxxxxx55555555555555555\n");
		#else
		panel_id = 0x02;
		printk("xxxxxxxxxxxxx4444444444\n");
		#endif
		...
		//panel_id = 0x00;
		printk("xxxxxxxxxxxxx33333\n");
		snprintf(filename, FILENAME_LEN_MAX, "%s_pid%02X%s",
			CY_FW_FILE_PREFIX, panel_id, CY_FW_FILE_SUFFIX);
		}
	printk("xxxxx %s: Filename: %s\n", __func__, filename);
...
static int upgrade_firmware_from_builtin(struct device *dev)
{
	...
	printk("xxxxx %s: Enabling firmware class loader built-in\n",__func__);
	...
	if (retval < 0) {
		printk("xxxx %s: Fail request firmware class file load\n",
			__func__);
		goto exit;
	}
		...
		const u8 *ttconfig_data, int ttconfig_size)
{
	...
	//if (ld->loader_pdata && (ld->loader_pdata->flags & CY_LOADER_FLAG_CALIBRATE_AFTER_TTCONFIG_UPGRADE)) {
	{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 13, 0))
		reinit_completion(&ld->calibration_complete);
#else
...
//	struct cyttsp5_touch_config *ttconfig;
	...
	panel_id = cyttsp5_get_panel_id(dev);
	dev_err(dev, "%s: Panel ID=%d ========\n", __func__, panel_id);
	if (panel_id == PANEL_ID_NOT_ENABLED)
		return ld->loader_pdata->ttconfigs[1];
	else if (panel_id == 0)
		return ld->loader_pdata->ttconfigs[0];
	else if (panel_id == 1) 		
		return ld->loader_pdata->ttconfig;
